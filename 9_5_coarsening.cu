...
__global__ void histo_private_kernel_2(char *data, unsigned int length, unsigned int *histo) {
...
int main() {
    const unsigned int data_length = (1 << 30); // 1M elements
    char *data;
    ...
    histo_private_kernel_2<<<blocks, threads_per_block>>>(data, data_length, histo_gpu);
